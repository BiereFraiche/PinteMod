...
function community_welcome_monitor(initial_player)
{
    ...
    if (level.pintemod_welcome_enabled)
    {
        ...
        // BOIII v2.0.0 compatibility:
        // The custom welcome HUD (newclienthudelem/setText) interrupts the
        // player connection on BOIII v2.0.0. Keep the native iPrintLn delivery.
        self iprintln(
            ezz_admin_localization::text(self, "welcome_line")
        );
        community_append_file(
            "pintemod/logs/community.log",
            "[" + GetTime() + "] WELCOME_SENT | " +
            self.name + " | delivery=iprintln | hud=disabled | chat=disabled\n"
        );
    }
