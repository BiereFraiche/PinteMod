...
Les fichiers Remote Control sont volontairement rangés dans `boiii/tools/`, avec les autres outils opérateur Windows de PinteMod. Le même `PinteMod_LiveConsole.ps1` prend en charge le mode local et le mode distant en lecture seule via `-RemoteDataRoot`.
...
Le dossier `boiii/tools/runtime/`, les fichiers `boiii/tools/*.local.json` et le secret DPAPI Remote RCON restent également locaux.
